...
### ドキュメント
- **README**: 実画面のスクリーンショット（ホーム・解説・成績・模試）を `docs/images/readme/` に追加。
- **`npm run screenshots:readme`**: Playwright で上記 PNG を再生成するスクリプトを追加。

## [1.2.25] - 2026-04-19

### ドキュメント
- **README**: ヒーローバナー・学習フロー図（`docs/images/readme/*.svg`）を追加。スクリーンショット追加手順は `docs/images/readme/README.md` を参照。

## [1.2.24] - 2026-04-18

### 追加（コンテンツ）
- **問題を100問追加**（id 201–300）。各章 **+20問** で合計 **300問**（各章60問）。応用寄りのシナリオ・判断を多めにしたオリジナル問題（[Issue #76](https://github.com/junichi-muraoka/jcsqe-study-app/issues/76)）。
- `questions_extra5.js` を追加し、`explanations_exp3.js` に選択肢別解説を追加。`sw.js` のプリキャッシュを更新（`jcsqe-v28`）。

### 変更
- ホームの合格予測で「解答ボリューム」の目標を **300問** に合わせて調整（`app.js`）。
