...
| 2 | **Align Cloudflare Pages production branch** — Cloudflare API `PATCH .../pages/projects/{name}` で **`production_branch`** を **今の Git ブランチ**（本番は `master` / `main`、検証は `staging` / `develop`）に合わせる | ルート **`https://<プロジェクト>.pages.dev`** に本番デプロイが載るようにする（Direct Upload ではダッシュボードから `production_branch` を変えにくいため） |
| 3 | **`wrangler pages deploy _site --project-name=... --branch=...`** | 上と同じブランチ名でアップロード（プレビュー専用 URL だけに留めない） |

**本番（PRD）**の実行タイミングは **GitHub Release の Publish**（または `master` / `main` での手動 `workflow_dispatch`）。**`master` への push だけでは本番ワークフローは動かない**（[ut-qms](https://github.com/junichi-muraoka/ut-qms) と同様の安全策）。
...
**通常は GitHub Release から行う**（[release_process.md](./release_process.md)）。

1. **`master`** に反映したうえで、**Releases → Draft a new release** → タグ `vX.Y.Z` を作成し **Publish**。  
2. **Actions** の **Deploy Cloudflare Pages** が自動実行される。  
3. 緑になれば Cloudflare の本番 `*.pages.dev` が更新される。

**手動だけ試す**とき: Actions → **Deploy Cloudflare Pages** → **Run workflow** → ブランチ **`master`**（または **`main`**）を選んで実行（緊急用・検証用）。

### 検証（STG）

**`staging` または `develop` に push** すると同じワークフローが走る。または **Run workflow** で **`staging`** を選ぶ。

### 成功後の確認

- Cloudflare の **Workers & Pages** で **デプロイ履歴** と **`*.pages.dev`** を確認する。  
- ブラウザで URL を開き、アプリが表示されれば OK。失敗時は [トラブルシュート](#トラブルシュート)。
...
| **`https://master.<プロジェクト>.pages.dev` には出るのに、ルートの `https://<プロジェクト>.pages.dev` だけ「Nothing is here yet」** | **[deploy-cloudflare-pages.yml](../.github/workflows/deploy-cloudflare-pages.yml)** がデプロイ前に **API で `production_branch` を Git のブランチ名に合わせる**。まだ空なら Actions の **Align Cloudflare Pages production branch** のログを確認。トークンに **Pages の Edit** が無いと PATCH が失敗する。 |
...
**手元からだけ直したいとき（通常は不要）**  
ワークフローと同じ [Update Project](https://developers.cloudflare.com/api/resources/pages/subresources/projects/methods/edit/) を `curl` で叩く。`production_branch` はデプロイしている Git ブランチ（`master` / `main` / `staging` / `develop`）に合わせる。
